...
\begin{definition}
\textit{Точечное \(n\)-мерное евклидово пространство~\(\R^n\)}~--- множество
...
\[
    \lim_{n \rightarrow \infty} \rho(x, x_0) = 0, \; \text{т.\,e.} \; \forall
...
\Longleftrightarrow \exists n_0 \; \forall n \geq n_0 \rightarrow x_n = x_0\)
...
\begin{proof}
\Boxedrightarrow Пусть множество ограничено, т.\,e. \(\diam X = d < +\infty\).
...
\[
    x_k \rightarrow x_0 \; \text{в} \; \R^n \Longleftrightarrow \forall i = 1,
        2, \dots, n \quad \alpha_i^{(k)} \rightarrow \alpha_i^{(0)}.
\]
\end{statement}
...
k, m \; \rho(x_k, x_m) = 1\), т.\,e. \(\diam \hm = 1\)). Но сходящуюся
...
\begin{gather}
    ...
    \forall i = 1 \dots n \; \left|x_i^{(k)} - x_i^{(m)}\right| < \epsilon
        \text{, т.\,e.} \; \forall i = 1 \dots n \rightarrow \exists \lim_{k
        \rightarrow \infty} x_i^{(k)} = x_i^{(0)}
\end{gather}
...
\begin{definition}
Множество~\(X \subset M\) называется \textit{открытым}, если \(\Int X = X\),
т.\,e. любая точка \(X\) является внутренней.
\end{definition}
