...
Применение символики \textit{не регулируется строго}, её можно писать и произносить по-разному.

Множества обозначают заглавными латинскими буквами (\(A\), \(B\), \(C\), \(\dots\), \(Z\)),
элементы множеств~--- \textit{прописными латинскими буквами} (\(a\), \(b\), \(c\), \(\dots\), \(z\)).

Иногда для обозначения числовых множеств будут использоваться прописные греческие буквы (\(\alpha\),
\(\beta\), \(\gamma\), \(\dots\), \(\omega\)).
...
\begin{example}
<<множество \(A\) является подмножеством множества \(B\)>> по определению то же самое, что и <<для
любого \(x\), принадлежащего множеству \(A\), выполняется, что \(x\) принадлежит множеству \(B\)>>:
\[
...
\begin{example}
<<множество \(A\) равняется множеству \(B\)>> по определению то же самое, что и <<множество \(A\)
является подмножеством множества \(B\) \textbf{и} множество \(B\) является подмножеством множества
\(A\)>>:
\[
...
\begin{example}
<<Объединением можеств \(A\) и \(B\)>> называют множество, все элементы \(x\) которого принадлежат
множеству \(A\) \textbf{или} множеству \(B\):
\[
    \left[A \cup B\right] = \left\{x : \left[x \in A\right] \lor \left[x \in B\right]\right\}
\]
\end{example}

\begin{example}
<<Пересечением можеств \(A\) и \(B\)>> называют множество, все элементы \(x\) которого принадлежат
множеству \(A\) \textbf{и} множеству \(B\):
\[
    \left[A \cap B\right] = \left\{x : \left[x \in A\right] \& \left[x \in B\right]\right\}
\]
\end{example}
...
\[
    \left[A \setminus B\right] = \left\{x : \left[x \in A\right] \&
    \left[x \not\in B\right]\right\}
\]
\end{example}
...
существования~\(\exists\)~--- <<:>> (это правило не является строгим, символы стоит использовать в
...
\[
    \neg \mathfrak{B} = \left[\exists x \in X: \lvert x \rvert \ge \alpha\right]
\]
\end{example}
...
Обозначим \(\mathfrak{C} = \left[\exists \alpha > 0: \forall x \in X \mapsto \lvert x \rvert <
\alpha\right]\), тогда:
\[
    \neg \mathfrak{C} = \left[ \forall \alpha > 0 \; \exists x_\alpha \in X: \lvert x_\alpha \rvert
        \geq \alpha\right]
\]
...
Для любых рациональных чисел~\(a\) и~\(b\) имеет место только одно из соотношений: \(a = b\), \(a >
b\), \(b > a\).
\end{mtopic}
...
\(A_* =  \left\{a \colon \dfrac{1}{10^3} > a\right\}\),
\(A^* = \left\{a \colon a \geq \dfrac{1}{10^3}\right\}\)
\end{example}
...
\begin{example}
\label{contex1}
\(b, c \in \N : (b + 1)^2 > c > b^2\) (\(c\) не является квадратом какого-либо
натурального числа):
\begin{gather}
A_* = \left\{a : \left[a \leq 0\right] \lor \left[c > a^2\right]\right\} \\
A^* = \left\{a : \left[a > 0\right] \& \left[a^2 \geq c\right]\right\}
\end{gather}
\end{example}

\begin{proposition}
Сечение \(\left(A_*, A^*\right)\) примера \ref{contex1} таково, что \textit{нижний} класс сечения не имеет
\textit{наибольшего} числа, а \textit{верхний} класс сечения не имеет \textit{наибольшего} числа.
\end{proposition}

\begin{proposition}
\textit{Не существует} \(\left(A_*, A^*\right)\) такого, что \(A_*\) имеет \textit{наибольшее} число,
а \(A^*\) имеет \textit{наименьшее} число (так как нарушится или первое, или второе условие из
определения сечения).
\end{proposition}
...
Так как сечения \(1^\circ\) и \(2^\circ\) определяются одним и тем же рациональным числом, то будем
использовать сечение \(1^\circ\).

\begin{definition}
\textit{Иррациональным числом} назовем сечение вида \(3^\circ\).
\end{definition}

Множество иррациональных чисел обозначается как \(\I\), а рациональные числа будем записывать через
греческие буквы \(\alpha = \left(A_*, A^*\right)\).
...
\begin{proposition}
\(\left[\alpha \not = \beta\right] \Rightarrow \left[A_* \subset B_*\right] \lor \left[B_* \subset
    A_*\right].\)
\end{proposition}

\begin{definition}
\(\left[\alpha > \beta\right] \eqdef \left[B_* \subset A_*\right] \& \left[B_* \not = A_*\right].\)
\end{definition}
